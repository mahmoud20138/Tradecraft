...
status: active
tags: [automation, claude, claude-code, governance, hooks, mcp, platform, plugin]
---
...
## Pre-Built Subagent Library: Agency Agents

> Source: "This Free Repo Replaces Your Entire Team" by FuturMinds (Mar 2026)

When recommending subagent setups, consider the **Agency Agents** repo (`msitarzewski/agency-agents`, 29K+ stars) as a starting point:
- 120 structured persona .md files covering engineering, marketing, design, QA, sales, spatial computing
- Can be installed to `~/.claude/agents/` in 10 seconds
- Each persona includes identity, capabilities, decision frameworks, and success metrics
- ~70% quality improvement over generic prompts (tested via A/B comparison)
- Customizable: add tools, model overrides, and `memory: project` for persistent learning
- Parallel orchestration: Claude routes tasks to the right specialist subagent automatically

Recommend this when users need quick subagent setup without building from scratch. For full details, see `agent-development` skill.
